...
- **SKILL_MAP.md:** Read once, at Step 3 (generation plan). Never before, never again after.
- **Each SKILL.md:** Read at the start of that specific generation stage only.
- **Stack skills:** Read ONE — the one matching the user's Q5 answer. Skip all others.
- **Partial sessions:** If only generating 1–2 docs, only read the skills for those docs. Do not preload the full bundle's skills.

This prevents wasting context window on skills that won't be used in the current session.
